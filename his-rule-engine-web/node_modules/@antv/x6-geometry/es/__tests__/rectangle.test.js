...
describe('rectangle', () => {
    ...
    describe('#Rectangle.isRectangleLike', () => {
        it('should return true if the given object is a rectangle-like object', () => {
            ...
            expect(Rectangle.isRectangleLike(Object.assign(Object.assign({}, obj), { z: 10 }))).toBeTruthy();
            expect(Rectangle.isRectangleLike(Object.assign(Object.assign({}, obj), { z: 10, s: 's' }))).toBeTruthy();
        });
    ...
    describe('#Rectangle.fromPositionAndSize', () => {
        it('should create a rectangle from the given position and size', () => {
            expect(Rectangle.fromPositionAndSize({ x: 2, y: 5 }, { width: 10, height: 8 }).toJSON()).toEqual({
                x: 2,
                y: 5,
                width: 10,
                height: 8,
    ...
    describe('#bbox', () => {
        ...
        it('should rotate the rectangle if angle specified.', () => {
            ...
            expect(rect.bbox(90).round().equals(new Rectangle(-1, 1, 4, 2))).toBeTruthy();
        });
    });
    describe('#add', () => {
    ...
    describe('#scale', () => {
        ...
        it('should scale point with the given amount and center ', () => {
            expect(new Rectangle(20, 30, 10, 20).scale(2, 3, new Point(40, 45)).toJSON()).toEqual({ x: 0, y: 0, width: 20, height: 60 });
        });
    });
    describe('#rotate', () => {
    ...
    describe('#moveAndExpand', () => {
        it('should translate and expand me by the given `rect`', () => {
            expect(new Rectangle(1, 2, 3, 4)
                .moveAndExpand(new Rectangle(1, 2, 3, 4))
                .toJSON()).toEqual({ x: 2, y: 4, width: 6, height: 8 });
            expect(new Rectangle(1, 2, 3, 4).moveAndExpand(new Rectangle()).toJSON()).toEqual({ x: 1, y: 2, width: 3, height: 4 });
        });
    });
    ...
    describe('#containsRect', () => {
        it('should return true when rect is completely inside the other rect', () => {
            expect(new Rectangle(50, 50, 100, 100).containsRect(60, 60, 80, 80)).toBeTruthy();
        });
        it('should return true when rect is equal the other rect', () => {
            expect(new Rectangle(50, 50, 100, 100).containsRect({
                x: 50,
                y: 50,
                width: 100,
                height: 100,
        ...
        it('should return false when rect is not inside the other rect', () => {
            expect(new Rectangle(50, 50, 100, 100).containsRect(20, 20, 200, 200)).toBeFalsy();
            expect(new Rectangle(50, 50, 100, 100).containsRect(40, 40, 100, 100)).toBeFalsy();
            expect(new Rectangle(50, 50, 100, 100).containsRect(60, 60, 100, 40)).toBeFalsy();
            expect(new Rectangle(50, 50, 100, 100).containsRect(60, 60, 100, 100)).toBeFalsy();
            expect(new Rectangle(50, 50, 100, 100).containsRect(60, 60, 40, 100)).toBeFalsy();
        });
        it('should return false when one of the dimensions is `0`', () => {
            expect(new Rectangle(50, 50, 100, 100).containsRect(75, 75, 0, 0)).toBeFalsy();
    ...
    describe('#intersectsWithRect', () => {
        ...
        it('should return null when no intersection', () => {
            expect(new Rectangle(20, 20, 100, 100).intersectsWithRect([140, 140, 20, 20])).toBeNull();
        });
    });
    describe('#intersectsWithLine', () => {
        it('should return the intersection points', () => {
            const points1 = new Rectangle(0, 0, 4, 4).intersectsWithLine(new Line(2, 2, 2, 8));
            const points2 = new Rectangle(0, 0, 4, 4).intersectsWithLine(new Line(2, -2, 2, 8));
            expect(Point.equalPoints(points1, [{ x: 2, y: 4 }]));
            expect(Point.equalPoints(points2, [
                { x: 2, y: 0 },
                { x: 2, y: 4 },
            ]));
        });
        it('should return null when no intersection exists', () => {
            expect(new Rectangle(0, 0, 4, 4).intersectsWithLine(new Line(-2, -2, -2, -8))).toBeNull();
        });
    });
    describe('#intersectsWithLineFromCenterToPoint', () => {
        it('should return the intersection point', () => {
            var _a, _b;
            expect((_a = new Rectangle(0, 0, 4, 4)
                .intersectsWithLineFromCenterToPoint([2, 8])) === null || _a === void 0 ? void 0 : _a.equals([2, 4])).toBeTruthy();
            expect((_b = new Rectangle(0, 0, 4, 4)
                .intersectsWithLineFromCenterToPoint([2, 8], 90)) === null || _b === void 0 ? void 0 : _b.round().equals([2, 4])).toBeTruthy();
        });
        it('should return null when no intersection exists', () => {
            expect(new Rectangle(0, 0, 4, 4).intersectsWithLineFromCenterToPoint([3, 3])).toBeNull();
        });
    });
    describe('#normalize', () => {
