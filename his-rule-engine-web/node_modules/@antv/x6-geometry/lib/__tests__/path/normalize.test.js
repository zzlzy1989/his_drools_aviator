...
describe('Path', () => {
    describe('#normalizePathData', () => {
        ...
        it('should normalize path data', () => {
            paths.forEach((path) => {
                if (path[1]) {
                    expect((0, path_1.normalizePathData)(path[0])).toEqual(path[1]);
                }
                else {
                    (0, path_1.normalizePathData)(path[0]);
                }
            });
        });
        it('should parsed by Path', () => {
            const path1 = 'M 10 10';
            const normalizedPath1 = (0, path_1.normalizePathData)(path1);
            const reconstructedPath1 = path_1.Path.parse(path1).serialize();
            expect(normalizedPath1).toEqual(reconstructedPath1);
            const path2 = 'M 100 100 C 100 100 0 150 100 200 Z';
            const normalizedPath2 = (0, path_1.normalizePathData)(path2);
            const reconstructedPath2 = path_1.Path.parse(path2).serialize();
            expect(normalizedPath2).toEqual(reconstructedPath2);
            const path3 = 'M285.8,83V52.7h8.3v31c0,3.2-1,5.8-3,7.7c-2,1.9-4.4,2.8-7.2,2.8c-2.9,0-5.6-1.2-8.1-3.5l3.8-6.1c1.1,1.3,2.3,1.9,3.7,1.9c0.7,0,1.3-0.3,1.8-0.9C285.5,85,285.8,84.2,285.8,83z';
            const normalizedPath3 = (0, path_1.normalizePathData)(path3);
            const reconstructedPath3 = path_1.Path.parse(path3).serialize();
