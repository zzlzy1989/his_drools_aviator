...
class MiniMap extends x6_1.View {
    get scroller() {
    ...
    init(graph) {
        this.graph = graph;
        this.updateViewport = x6_1.FunctionExt.debounce(this.updateViewport.bind(this), 0);
        this.container = document.createElement('div');
        x6_1.Dom.addClass(this.container, this.prefixClassName(ClassName.root));
        const graphContainer = document.createElement('div');
        this.container.appendChild(graphContainer);
        this.viewport = document.createElement('div');
        x6_1.Dom.addClass(this.viewport, this.prefixClassName(ClassName.viewport));
        if (this.options.scalable) {
            this.zoomHandle = document.createElement('div');
            x6_1.Dom.addClass(this.zoomHandle, this.prefixClassName(ClassName.zoom));
            x6_1.Dom.appendTo(this.zoomHandle, this.viewport);
        }
        x6_1.Dom.append(this.container, this.viewport);
        ...
        this.targetGraph = this.options.createGraph
            ...
            : new x6_1.Graph(targetGraphOptions);
        this.updatePaper(this.sourceGraph.options.width, this.sourceGraph.options.height);
        this.startListening();
    }
    startListening() {
        if (this.scroller) {
            x6_1.Dom.Event.on(this.graphContainer, `scroll${this.getEventNamespace()}`, this.updateViewport);
        }
        else {
    ...
    stopListening() {
        if (this.scroller) {
            x6_1.Dom.Event.off(this.graphContainer, this.getEventNamespace());
        }
        else {
    ...
    updateViewport() {
        ...
        const position = x6_1.Dom.position(this.targetGraph.container);
        ...
        x6_1.Dom.css(this.viewport, this.geometry);
    }
    startAction(evt) {
    ...
    scrollTo(evt) {
        ...
        ts.ty = ts.ty || 0;
        if (e.offsetX == null) {
            const offset = x6_1.Dom.offset(this.targetGraph.container);
...
(function (Util) {
    Util.defaultOptions = {
        ...
        maxScale: 16,
        graphOptions: {},
        createGraph: (options) => new x6_1.Graph(options),
    };
    Util.documentEvents = {
        mousemove: 'doAction',
        touchmove: 'doAction',
