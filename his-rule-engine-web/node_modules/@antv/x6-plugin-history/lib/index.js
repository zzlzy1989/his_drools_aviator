...
require("./api");
class History extends x6_1.Basecoat {
    constructor(options = {}) {
        super();
        this.name = 'history';
    ...
    revertCommand(cmd, options) {
        ...
        for (let i = cmds.length - 1; i >= 0; i -= 1) {
            const cmd = cmds[i];
            const localOptions = Object.assign(Object.assign({}, options), x6_1.ObjectExt.pick(cmd.options, this.options.revertOptionsList || []));
    ...
    applyCommand(cmd, options) {
        ...
        for (let i = 0; i < cmds.length; i += 1) {
            const cmd = cmds[i];
            const localOptions = Object.assign(Object.assign({}, options), x6_1.ObjectExt.pick(cmd.options, this.options.applyOptionsList || []));
    ...
    executeCommand(cmd, revert, options) {
        ...
        if ((Util.isAddEvent(event) && revert) ||
            ...
            if (executeCommand) {
                x6_1.FunctionExt.call(executeCommand, this, cmd, revert, options);
            }
        }
    }
    addCommand(event, args) {
        ...
        if (before != null &&
            x6_1.FunctionExt.call(before, this, event, args) === false) {
            return;
        }
        ...
        const isModelChange = x6_1.Model.isModel(cell);
        let cmd;
        ...
        if (Util.isAddEvent(event) || Util.isRemoveEvent(event)) {
            ...
            cmd.options = options;
            data.id = cell.id;
            data.props = x6_1.ObjectExt.cloneDeep(cell.toJSON());
            if (cell.isEdge()) {
                data.edge = true;
            }
            else if (cell.isNode()) {
        ...
        if (Util.isChangeEvent(event)) {
            ...
            if (!cmd.batch || !cmd.event) {
                ...
                data.prev[key] = x6_1.ObjectExt.cloneDeep(cell.previous(key));
                if (isModelChange) {
            ...
            data.next[key] = x6_1.ObjectExt.cloneDeep(cell.prop(key));
            return this.push(cmd, options);
        }
        // others
        // ------
        ...
        if (afterAddCommand) {
            x6_1.FunctionExt.call(afterAddCommand, this, event, args, cmd);
        }
        ...
        while (cmds.length > 0) {
            ...
            if (evt != null && (id != null || cmd.modelChange)) {
                if (Util.isAddEvent(evt)) {
                    ...
                    const data = cmd.data;
                    if (x6_1.ObjectExt.isEqual(data.prev, data.next)) {
                        continue;
                    }
                }
                else {
...
(function (History) {
     ...
     */
    class Validator extends x6_1.Basecoat {
        constructor(options) {
            super();
            this.map = {};
