...
class Dnd extends x6_1.View {
    ...
    init() {
        ...
        x6_1.Dom.addClass(this.container, this.prefixClassName('widget-dnd'));
        this.draggingGraph = new x6_1.Graph(Object.assign(Object.assign({}, this.options.delegateGraphOptions), { container: document.createElement('div'), width: 1, height: 1, async: false }));
        x6_1.Dom.append(this.container, this.draggingGraph.container);
    }
    start(node, evt) {
        const e = evt;
        e.preventDefault();
        this.targetModel.startBatch('dnd');
        x6_1.Dom.addClass(this.container, 'dragging');
        x6_1.Dom.appendTo(this.container, this.options.draggingContainer || document.body);
    ...
    onDragEnd(evt) {
        ...
        if (draggingNode) {
            ...
            if (x6_1.FunctionExt.isAsync(ret)) {
                // stop dragging
    ...
    onDropped(draggingNode) {
        if (this.draggingNode === draggingNode) {
            this.clearDragging();
            x6_1.Dom.removeClass(this.container, 'dragging');
            x6_1.Dom.remove(this.container);
        }
    }
    onDropInvalid() {
        const draggingNode = this.draggingNode;
    ...
    getDropArea(elem) {
        const offset = x6_1.Dom.offset(elem);
        const scrollTop = document.body.scrollTop || document.documentElement.scrollTop;
        const scrollLeft = document.body.scrollLeft || document.documentElement.scrollLeft;
        return x6_1.Rectangle.create({
            x: offset.left +
                parseInt(x6_1.Dom.css(elem, 'border-left-width'), 10) -
                scrollLeft,
            y: offset.top +
                parseInt(x6_1.Dom.css(elem, 'border-top-width'), 10) -
                scrollTop,
    ...
    drop(draggingNode, pos) {
        if (this.isInsideValidArea(pos)) {
            ...
            droppingNode.position(x6_1.GeometryUtil.snapToGrid(local.x, gridSize), x6_1.GeometryUtil.snapToGrid(local.y, gridSize));
            ...
            return x6_1.FunctionExt.toDeferredBoolean(ret).then((valid) => {
                if (valid) {
                    targetModel.addCell(droppingNode, { stencil: this.cid });
                    return droppingNode;
                }
