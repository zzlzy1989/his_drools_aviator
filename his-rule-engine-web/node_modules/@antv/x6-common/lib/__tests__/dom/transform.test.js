...
describe('Dom', () => {
    ...
    describe('#getTransformToElement', () => {
        it('rotate', () => {
            ...
            const group = vector_1.Vector.create('g');
            const rect = vector_1.Vector.create('rect');
            container.append(group);
            container.append(rect);
            rect.rotate(45);
            const matrix = dom_1.Dom.getTransformToElement(group.node, rect.node);
            expect({
    ...
    describe('#getTransformToParentElement', () => {
        const isSimilar = (matrix1, matrix2) => {
            const result = [
        ...
        it('translate', () => {
            const container = vector_1.Vector.create(svgContainer);
            const group = vector_1.Vector.create('g');
            const node = vector_1.Vector.create('g');
            const rect = vector_1.Vector.create('rect');
            container.append(group);
            group.append(node);
            node.append(rect);
            ...
            const matrix = dom_1.Dom.getTransformToParentElement(rect.node, node.node);
            const result = dom_1.Dom.getTransformToElement(rect.node, node.node);
        ...
        it('rotate', () => {
            ...
            node.rotate(10);
            rect.rotate(20);
            const matrix = dom_1.Dom.getTransformToParentElement(rect.node, group.node);
            const result = dom_1.Dom.getTransformToElement(rect.node, group.node);
            expect(isSimilar(matrix, result)).toEqual(true);
            rect.remove();
            node.remove();
            group.remove();
