    ...
    describe('attr', () => {
        describe('#styleToObject', () => {
            it('should parse style string to object', () => {
                ...
                expect(dom_1.Dom.styleToObject('fill=red; stroke=blue')).toEqual(ret);
                expect(dom_1.Dom.styleToObject('fill=red; stroke=blue;')).toEqual(ret);
            });
            it('should parse key-only string', () => {
                expect(dom_1.Dom.styleToObject(';fill=red;;stroke')).toEqual({
                    fill: 'red',
                    stroke: '',
                });
            });
            it('should ingore empty section', () => {
                expect(dom_1.Dom.styleToObject(';fill=red;;')).toEqual({ fill: 'red' });
            });
            it('should parse empty string to empty object', () => {
                expect(dom_1.Dom.styleToObject('')).toEqual({});
            });
        });
        describe('#mergeAttrs', () => {
            it('shoule merge attrs by extend', () => {
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10 }, { y: 20 })).toEqual({
                    x: 5,
                    y: 20,
                });
            });
            it('should append class attribute', () => {
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10, class: 'foo' }, { y: 20, class: 'bar' })).toEqual({ x: 5, y: 20, class: 'foo bar' });
            });
            it('shoule merge style object', () => {
                const result = { x: 5, y: 20, style: { fill: 'red', stroke: 'orange' } };
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: 'stroke=orange' })).toEqual(result);
                expect(dom_1.Dom.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: 'stroke=orange' })).toEqual(result);
            });
        ...
        describe('#attr', () => {
            it('should get all attributes', () => {
                ...
                vel.setAttribute('foo', 'test');
                vel.setAttribute('bar', '100');
                const _a = vel.attr(), { id } = _a, attrs = __rest(_a, ["id"]);
            ...
            it('should work on html element', () => {
                ...
                dom_1.Dom.attr(div, 'data-id', '12');
                expect(dom_1.Dom.attr(div, 'data-id')).toEqual('12');
            });
        });
    });
});
