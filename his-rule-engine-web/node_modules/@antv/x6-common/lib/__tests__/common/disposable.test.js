...
class AOPTest extends disposable_1.Disposable {
    constructor() {
        super(...arguments);
        this.a = 1;
    }
...
describe('disposable', () => {
    describe('Disablable', () => {
        it('should be `false` before object is disposed', () => {
            const obj = new disposable_1.Disposable();
    ...
    describe('DisposableDelegate', () => {
        describe('#constructor', () => {
            it('should accept a callback', () => {
                const delegate = new disposable_1.DisposableDelegate(() => { });
        ...
        describe('#dispose', () => {
            it('should invoke a callback when disposed', () => {
                let called = false;
                const delegate = new disposable_1.DisposableDelegate(() => (called = true));
            ...
            it('should ignore multiple calls to `dispose`', () => {
                let count = 0;
                const delegate = new disposable_1.DisposableDelegate(() => (count += 1));
                expect(count).toBe(0);
                delegate.dispose();
                delegate.dispose();
                delegate.dispose();
    ...
    describe('DisposableSet', () => {
        describe('#constructor', () => {
            it('should accept no arguments', () => {
                const set = new disposable_1.DisposableSet();
        ...
        describe('#dispose', () => {
            it('should dispose all items in the set', () => {
                ...
                const set = disposable_1.DisposableSet.from([item1, item2, item3]);
            ...
            it('should dipose items in the order they were added', () => {
                const values = [];
                const item1 = new disposable_1.DisposableDelegate(() => values.push(0));
                const item2 = new disposable_1.DisposableDelegate(() => values.push(1));
                const item3 = new disposable_1.DisposableDelegate(() => values.push(2));
                const set = disposable_1.DisposableSet.from([item1, item2, item3]);
        ...
        describe('#add', () => {
            ...
            it('should maintain insertion order', () => {
                ...
                const item2 = new disposable_1.DisposableDelegate(() => values.push(1));
                const item3 = new disposable_1.DisposableDelegate(() => values.push(2));
                const set = disposable_1.DisposableSet.from([item1]);
                set.add(item2);
                set.add(item3);
                expect(values).toEqual([]);
                set.dispose();
        ...
        describe('#remove', () => {
            ...
            it('should ignore missing items', () => {
                ...
                const item2 = new disposable_1.DisposableDelegate(() => values.push(1));
                const item3 = new disposable_1.DisposableDelegate(() => values.push(2));
                const set = disposable_1.DisposableSet.from([item1, item2]);
                expect(values).toEqual([]);
                set.remove(item3);
                set.dispose();
