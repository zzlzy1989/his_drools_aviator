    ...
    describe('matrix', () => {
        ...
        describe('#parseTransformString', () => {
            it('should parse scale, rotate, translate', () => {
                const parsed = Dom.parseTransformString('scale(3) rotate(6) translate(9) xxx(11)');
        ...
        describe('#transformStringToMatrix', () => {
            ...
            arr.forEach((transformString) => {
                it(`should convert "${transformString}" to matrix`, () => {
                    ...
                    expect(Dom.transformStringToMatrix(transformString)).toEqual(svgTestGroup.node.getCTM());
                });
            });
        });
        describe('#matrixToTransformString', () => {
            it('should return correct transformation string', () => {
                ...
                expect(Dom.matrixToTransformString({ a: 2, d: 2 })).toEqual('matrix(2,0,0,2,0,0)');
                expect(Dom.matrixToTransformString({ a: 1, b: 2, c: 3, d: 4, e: 5, f: 6 })).toEqual('matrix(1,2,3,4,5,6)');
                expect(Dom.matrixToTransformString(Dom.createSVGMatrix({ a: 1, b: 2, c: 3, d: 4, e: 5, f: 6 }))).toEqual('matrix(1,2,3,4,5,6)');
                expect(Dom.matrixToTransformString({ a: 0, b: 1, c: 1, d: 0, e: 0, f: 0 })).toEqual('matrix(0,1,1,0,0,0)');
            });
        });
        describe('#matrixTo[Transformation]', () => {
            ...
            it('should convert matrix to rotation metadata', () => {
                ...
                angle = Dom.matrixToRotation(Dom.createSVGMatrix().translate(50, 50).rotate(15));
                expect(roundObject(angle)).toEqual({ angle: 15 });
                angle = Dom.matrixToRotation(Dom.createSVGMatrix().translate(50, 50).rotate(60).scale(2));
                expect(roundObject(angle)).toEqual({ angle: 60 });
                angle = Dom.matrixToRotation(Dom.createSVGMatrix().rotate(60).rotate(60));
                expect(roundObject(angle)).toEqual({ angle: 120 });
            });
            it('should convert matrix to translation medata', () => {
                let translate;
                translate = Dom.matrixToTranslation(Dom.createSVGMatrix().translate(10, 20));
                expect(roundObject(translate)).toEqual({ tx: 10, ty: 20 });
                translate = Dom.matrixToTranslation(Dom.createSVGMatrix().translate(10, 20).rotate(10, 20).scale(2));
                expect(roundObject(translate)).toEqual({ tx: 10, ty: 20 });
                translate = Dom.matrixToTranslation(Dom.createSVGMatrix().translate(10, 20).translate(30, 40));
            ...
            it('should convert matrix to scaling metadata', () => {
                ...
                scale = Dom.matrixToScale(Dom.createSVGMatrix()
                    .translate(15, 15)
                    .scaleNonUniform(2, 3)
                    .rotate(10, 20));
