    ...
    describe('attr', () => {
        ...
        describe('#mergeAttrs', () => {
            ...
            it('should append class attribute', () => {
                expect(Dom.mergeAttrs({ x: 5, y: 10, class: 'foo' }, { y: 20, class: 'bar' })).toEqual({ x: 5, y: 20, class: 'foo bar' });
            });
            it('shoule merge style object', () => {
                const result = { x: 5, y: 20, style: { fill: 'red', stroke: 'orange' } };
                expect(Dom.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(Dom.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(Dom.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: 'stroke=orange' })).toEqual(result);
                expect(Dom.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: 'stroke=orange' })).toEqual(result);
            });
