...
function rollup(points, merge) {
    if (merge != null && merge !== false) {
        ...
        if (amount > 0) {
            const center1 = x6_geometry_1.Point.create(points[1]).move(points[2], amount);
            const center2 = x6_geometry_1.Point.create(points[1]).move(points[0], amount);
...
const loop = function (vertices, options, edgeView) {
    ...
    if (sourceAnchor.equals(targetAnchor)) {
        const getVertices = (angle) => {
            const rad = x6_geometry_1.Angle.toRad(angle);
            const sin = Math.sin(rad);
            const cos = Math.cos(rad);
            const center = new x6_geometry_1.Point(sourceAnchor.x + cos * width, sourceAnchor.y + sin * width);
            const ref = new x6_geometry_1.Point(center.x - cos * halfHeight, center.y - sin * halfHeight);
        ...
        const validate = (end) => {
            ...
            const line = new x6_geometry_1.Line(start, end);
    ...
    {
        const line = new x6_geometry_1.Line(sourceAnchor, targetAnchor);
        ...
        const line1 = new x6_geometry_1.Line(ref.start, center);
        const line2 = new x6_geometry_1.Line(ref.end, center);
        ...
        if (options.merge) {
            const line = new x6_geometry_1.Line(sourceAnchor, targetAnchor);
            const normal = new x6_geometry_1.Line(center, line.center).setLength(Number.MAX_SAFE_INTEGER);
