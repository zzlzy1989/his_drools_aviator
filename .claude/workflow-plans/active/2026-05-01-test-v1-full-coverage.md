...
### 1.1 背景
V1.0 功能开发已完成（P0 100%，P1 ~60%），需要进行全面的测试验证：
- 单元测试：覆盖核心规则、公式计算、Service 层
- 接口测试：覆盖所有 REST API，端到端验证
- 安全测试：SQL 注入、Aviator 表达式注入、权限控制
...
| 任务 | 模块 | 测试内容 | 目标覆盖率 |
|------|------|---------|-----------|
| 1.1 | Aviator 公式计算 | `AviatorHelperTest` - 所有已实现公式的单元测试 | ≥95% |
| 1.2 | Drools 规则 | `ReimbursementRuleTest` - 结算规则测试 | ≥90% |
| 1.3 | Skill Pipeline | `SkillPipelineExecutorTest` - 管道执行测试 | ≥90% |
| 1.4 | Service 层 | 各模块 Service 单元测试 | ≥80% |
| 1.5 | Controller 层 | 各模块 Controller 测试 | ≥70% |

### Phase 2: 接口测试

| 任务 | 模块 | 测试内容 | 输出格式 |
|------|------|---------|---------|
| 2.1 | 规则管理 API | 规则 CRUD + 发布 + 版本管理 | Postman Collection |
| 2.2 | 公式管理 API | 公式 CRUD + 校验 + 发布 | Postman Collection |
| 2.3 | 医保结算 API | 结算执行 + 查询 + 历史 | Postman Collection |
| 2.4 | 合理用药 API | 处方审核 | Postman Collection |
| 2.5 | 质控 API | 质控检查 | Postman Collection |
| 2.6 | DRG 分组 API | 分组 + 查询 | Postman Collection |
| 2.7 | 全链路测试 | 端到端结算流程 | Postman Collection |

### Phase 3: 安全测试

| 任务 | 测试内容 | 方法 |
|------|---------|------|
| 3.1 | SQL 注入防护 | 输入 `' OR '1'='1` 等注入语句 |
| 3.2 | Aviator 表达式注入 | 输入 `System.exit(1)` 等危险函数 |
| 3.3 | XSS 防护 | 输入 `<script>alert(1)</script>` |
| 3.4 | 权限控制 | 无 Token/错误 Token 访问受保护接口 |
| 3.5 | 租户隔离 | 跨租户数据访问验证 |
