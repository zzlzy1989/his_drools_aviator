...
### 3.2 任务开始前的必做动作（Hook: pre-task-start）
...
4. **检查活跃计划**: 读取 `.trae/workflow-plans/INDEX.md`，查看是否有相关活跃计划
   - 如有相关计划 → 在回答中显示计划状态提示
   - 如无相关计划 → 提示用户是否需要创建计划（使用 `((command:plan create ...))`）
5. 更新 `## 6. 当前任务上下文` 区域
6. 如有歧义，**先问再做**
...
```
📋 关联计划检查:
  [pending] 实现医保报销规则引擎 (feature) - Phase 2
  💡 提示: 当前任务是否属于此计划？如是，请更新状态为 in_progress
```

### 3.3 任务完成后的必做动作（Hook: post-task-complete）

**必须按顺序执行以下步骤**:

1. **更新关联计划状态**:
   - 识别当前任务关联的计划文件
   - 提示用户更新计划状态: `((command:plan update <文件名> completed))`
   - 如用户确认完成，直接修改计划文件的 `status` 为 `completed`，并设置 `completed_at` 时间

2. **执行知识回写三问**（可使用 `/knowledge-writeback`）:
   - 暴露了哪些未明确的业务假设？
   - 代码做了什么但没说清楚为什么？
   - 有没有和现有知识冲突？

3. **更新索引**: 如计划状态变更，同步更新 `.trae/workflow-plans/INDEX.md`

4. **记录变更日志**: 在 `.trae/CHANGELOG.md` 末尾添加完成记录

**计划更新输出格式**:
```
✅ 任务已完成
📋 请更新关联计划状态:
   ((command:plan update 2026-04-26-feature-xxx.md completed))
📝 建议执行知识回写 (Step 7)
```
