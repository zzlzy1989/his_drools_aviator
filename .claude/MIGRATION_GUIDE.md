...
> 将 TestHub 的 Harness 工程体系复用到任何 AI 辅助开发项目
...
通过在 `.claude/` 目录下建立 **四层结构化体系**，让 AI 助手：
...
```bash
# 1. 复制骨架目录到目标项目
cp -r source_project/.claude target_project/.claude

# 2. 删除 TestHub 特有内容（见第三节"需替换文件清单"）
...
| `hooks/post-task-complete.sh` | 知识回写触发 | ❌ 不需要改 |
...
| 文件 | 需要修改的内容 | 参考章节 |
|------|--------------|---------|
| `agent.md` | 项目名称/技术栈/硬性禁止项/组件规范 | §4.1 |
| `MEMORY.md` | 项目元信息/技术栈版本/目录结构/API/环境配置 | §4.2 |
| `settings.local.json` | 权限路径和受保护文件列表 | §4.3 |
...
| 文件 | 原因 | 替代方案 |
|------|------|---------|
| `HARNESS_IMPROVEMENT_PLAN.md` | TestHub 改进方案的详细记录 | 用你自己的规划文档替代 |
| `CHANGELOG.md` | TestHub 的变更历史 | 创建新的 CHANGELOG |
| `SUMMARY.md` | TestHub 的实施总结 | 本文档就是通用版总结 |
...
| 章节 | TestHub 内容 | 你的替换 |
|------|-------------|---------|
| Vue 组件结构 | Vue 3 + `<script setup>` | [你的前端框架规范] |
| TypeScript 规范 | TS 5.9 + 禁止 any | [你的类型规范] |
| 组件命名 | PascalCase/camelCase | [你的命名约定] |
| Django 后端 | Model/ViewSet/Serializer | [你的后端框架规范] 或删除 |
| Python 编码 | f-string/logging | [你的语言编码规范] 或删除 |
| i18n | t() 函数国际化 | [是否需要 i18n] |

**建议**: 如果你不使用 Django/Python，可以删除后端相关章节，保留前端部分。
...
- [ ] 复制 `hooks/post-task-complete.sh`（直接可用）
...
```bash
# 推荐: 将 .claude/ 加入 Git（排除敏感信息）
echo ".claude/settings.local.json" >> .gitignore  # 含个人权限配置
git add .claude/
...
- `.claude/agent.md`: AI 详细行为规范（面向 AI 的深度指令）
- 建议 CLAUDE.md 保持简洁（<100行），详细内容放 `.claude/` 目录
...
# 1. 创建目录结构
mkdir -p "${CLAUDE_DIR}"/{rules,commands,hooks,domain/modules,memory}

# 2. 创建 agent.md 最小模板
cat > "${CLAUDE_DIR}/agent.md" << 'AGENT_EOF'
...
# 3. 创建 MEMORY.md 最小模板
cat > "${CLAUDE_DIR}/MEMORY.md" << 'MEM_EOF'
...
# 4. 创建 memory 系统
cat > "${CLAUDE_DIR}/memory/TEMPLATE.md" << 'TPL_EOF'
# {date} 日记
...
echo "# memory/\n\n日记系统。每日一个文件。\n\n## 使用方式\n1. 复制 TEMPLATE.md 为 YYYY-MM-DD.md\n2. 记录当日重要发现\n3. 定期整理到 domain/" > "${CLAUDE_DIR}/memory/README.md"
...
*最后更新: 2026-04-25 | 基于 TestHub Platform V2.0 Harness 工程实践*
